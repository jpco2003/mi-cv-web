...
import { Code2, Cpu, Database, Globe as GlobeIcon, Layers, Server } from 'lucide-react'; 
import { translations } from '../translations'; // <-- IMPORTAMOS TU DICCIONARIO

export const Hero = ({ setSection, lang }) => {
  
  // Extraemos los textos del Hero según el idioma seleccionado
  ...
  const techStack = [
    ...
    { name: 'API REST / Integraciones', icon: <GlobeIcon size={16} />, color: 'group-hover:text-purple-400 group-hover:border-purple-400/50 group-hover:bg-purple-400/10' },
  ...
  return (
    <section className="relative flex flex-col items-center justify-center min-h-[85vh] px-4 text-center overflow-hidden animate-in fade-in duration-700">
      ...
      {/* TEXTO DINÁMICO DESDE EL DICCIONARIO */}
      <p className="text-xl md:text-2xl text-gray-400 max-w-2xl mx-auto leading-relaxed font-medium">
        {t.subtitle.split('Zoho One')[0]}<span className="text-white font-semibold">Zoho One</span>{t.subtitle.split('Zoho One')[1]}
      </p>
      ...
      <div className="mt-20 pt-10 w-full max-w-5xl relative">
        ...
        <p className="text-sm text-gray-500 mb-8 uppercase tracking-widest font-semibold">{t.stack}</p>
